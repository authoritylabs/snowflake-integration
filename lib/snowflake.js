...
async function createStage({ database, schema, bucket, storageIntegration }) {
  const connection = await createConnection();
  await connection.execute(`USE SCHEMA ${database}.${schema};`);
  const stageName = 'SERPWOW_RESULTS_S3_STAGE';

  const statement = `CREATE STAGE ${stageName}
  URL = 's3://${bucket}'
  STORAGE_INTEGRATION = ${storageIntegration};`;

  const stage = await connection.execute(statement);
...
export default { createS3StorageIntegration, createStage, testCredentials };
