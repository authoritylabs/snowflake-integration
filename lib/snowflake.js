...
async function createS3StorageIntegration({ bucketName, roleArn }) {
  const connection = await createConnection();
  const integrationName = 'SERPWOW_RESULTS_S3';

  const statement = `CREATE STORAGE INTEGRATION ${integrationName}
    TYPE = EXTERNAL_STAGE
    STORAGE_PROVIDER = 'S3'
    ENABLED = TRUE
  ...
  const integration = await connection
    .execute(`DESC INTEGRATION ${integrationName}`)
    .then((props) => {
