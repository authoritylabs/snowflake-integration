...
const STORAGE_INTEGRATION_NAME = 'VALUESERP_RESULTS_S3';
...
async function createPipe(options) {
  ...
  const pipeName = `${database}.${schema}.VALUESERP_RESULTS_PIPE`;
...
async function createStage(options) {
  ...
  const connection = await createConnection();
  const stageName = `${database}.${schema}.VALUESERP_RESULTS_S3_STAGE`;
...
async function createTable(database, schema) {
  const connection = await createConnection();
  const tableName = `${database}.${schema}.VALUESERP_RESULTS`;
