...
async function handleBucketCreatedState(state) {
  console.log(chalk.yellow('Creating IAM user for VALUE SERP'));
...
async function handleSerpWowPolicyCreatedState(state) {
  ...
  console.log(chalk.yellow('Attaching IAM S3 policy to VALUE SERP user'));
...
async function handleSerpWowPolicyAttachedState(state) {
  ...
  console.log(chalk.yellow('Creating access key for VALUE SERP user'));
...
function handleCreateDestinationError(error) {
  console.log(chalk.red(`Failed to create VALUE SERP destination: ${error.message}`));
...
async function handleSnowflakePolicyAttachedState(state) {
  console.log(chalk.yellow('Creating VALUE SERP S3 destination'));
...
function getCreateViewErrorMessage(sqlState) {
  if (sqlState === '02000') {
    return 'Verify that both the database and schema exist and the provided user has sufficient privileges then try again';
  }

  if (sqlState === '42710') {
    return 'Delete the view from Snowflake then try again';
  }

  return 'Verify privileges for Snowflake user then try again';
}

function handleCreateViewError(error) {
  console.log(chalk.red(`Failed to create Snowflake view: ${error.message}`));
  const message = getCreateViewErrorMessage(error.sqlState);

  throw setupFailedError(message);
}

async function handlePipeCreatedState(state) {
  const { database, schema, table } = state;

  const view = await snowflake
    .createView(database, schema, table.name)
    .catch(handleCreateViewError);
...
async function handleViewCreatedState(state) {
...
async function processNextSetupState(currentState) {
  const stateProgression = {
    ...
    VIEW_CREATED: handleViewCreatedState,
...
async function promptNewSetup() {
  const question = {
    type: 'input',
    name: 's3-bucket',
    message: 'Enter the name of the S3 bucket to be created where VALUE SERP results will be staged:',
    validate: validateRequiredInput,
  };
