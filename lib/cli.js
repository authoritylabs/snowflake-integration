...
function maskKey(key) {
  ...
  return key.slice(0, -4).replace(/./g, '*') + key.slice(-4);
}

function printCredsBoxText() {
  const { aws, serpWow, snowflake } = getCredentials();

  const blue = chalk.hex('4ca3d7');
  const lightBlue = chalk.hex('95d8f0');
  ...
  const splashText = [
    ...
    `${blue('VALUE SERP API key:')} ${maskKey(serpWow?.apiKey) ?? missing}`,
    `${lightBlue('Snowflake auth method:')} ${snowflake?.authMethod ?? missing}`,
    `${lightBlue('Snowflake account:')} ${snowflake?.account ?? missing}`,
    `${lightBlue('Snowflake username:')} ${snowflake?.username ?? missing}`,
  ].join('\n');
...
async function promptMainMenu() {
  const question = {
    ...
    choices: [
      ...
      {
        name: 'Setup VALUE SERP to Snowflake integration',
  ...
  await inquirer
    ...
    .then(async ({ 'main-menu': answer }) => {
      ...
      if (answer === 'setup-serp-to-snow-integration') {
        await serpToSnowSetup.promptSetup();
        await cliHelper.promptToContinue();
      }
...
const cli = {
  async start() {
    console.log('This tool will help set up a data ingestion pipeline between VALUE SERP Batches and SnowFlake');
